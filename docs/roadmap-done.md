...
**自动同步（默认关）。** 手动同步太繁琐——每改一次都要在这台按一下、再去另一台按一下。
现在两个时钟：改动后 **10 秒**防抖推送本机的编辑（连续编辑合并成一次），另外每 **5 分钟**
跑一次——后者是唯一会发现*另一台机器*上的编辑的东西。脏标记打在 `persist_profiles` 里，
因为所有配置改动本来就都从那儿过。

这条原本在「考虑过，不做」里，理由很具体：**这个项目唯一一次批量删除的 bug，是因为有人
在决定何时按按钮才被拦住的。** 定时器把这个人拿掉了，所以那道保护必须从"碰巧有人在场"变
成显式检查——**无人值守的合并若要删除超过 3 个本机会话，就暂停并询问**，而不是直接应用。
不设成 0：另一台机器上删掉一个会话本来就该同步过来，每次都拦这功能就没法用了；这个阈值
是用来区分"我在那边整理了一下"和"出事了"。

数量是**对着本机当前列表现算**的，不是读合并自己报的 `stats.deleted`——真正要紧的是这台
机器上有多少东西即将消失，而这么算不依赖合并的账目本身是对的。**手动同步永不被拦**：按
按钮本身就是那个决定，这一点没变。

路上发现两个问题。**自喂循环**：应用合并会走 `persist_profiles`，而它现在会标脏，不清掉
的话一次同步会安排下一次、无限循环，每次都发现没东西可传。以及
**`prune_profile_selection` 的注释在撒谎**——它写着自己覆盖"同步替换整个目录"的情况，但
`SyncFinished` 从来没调过它，于是同步删掉配置后选中集合里会留下死 id。自动同步让目录替换
变得频繁且无人值守，这个既有缺口更容易撞上，两条路径都补了。

**RDP：失焦时补发修饰键抬起。** 报上来的现象是"用着用着键盘就乱了"。抓到现行——helper
日志停在 `scancode=29 pressed=true`（左 Ctrl）和 `scancode=42 pressed=true`（左 Shift），
两个都只有按下没有抬起，远端于是一直以为它们按着，之后每一次击键都变成 `Ctrl+Shift+某键`。

根因是**完全没有失焦处理**（窗口订阅只有 Opened/Resized/FileDropped）。全局热键由 Windows
直接派发给注册它的程序**并夺走焦点**，所以按下到得了 Adit、抬起永远到不了。截图工具的
`Ctrl+Shift+X` 正是如此——这和更早那条"`Ctrl+Shift+X` 窗口模式下不透传"是同一个机制的两
面；当时只解释了不透传的那一半，漏掉了它还会留下卡死的修饰键，而后者才是真正伤人的。

抬起是**无条件发**的，不按自己记的状态发：给一个没按下的键发抬起在远端是空操作，而"自己
维护一份和远端不同步的状态"正是这个 bug 本身。

**编辑器「连接」按钮会保存并关窗了。** 之前点了连接会连上，但对话框留在原地挡着它刚打开
的那个会话，看起来像什么都没发生。只在编辑器确实打开时才保存（同一个消息也来自侧边栏和菜
单，那时表单里是上次加载的内容）；校验失败则保留对话框并显示错误，而不是拿无效值去连。

**helper 日志不再互相抹除。** 原本每次启动都用 `fs::write` 截断，而每个 RDP 会话各起一个
helper、共用同一个文件——开第二个会话就把第一个的日志清空了。这直接毁掉过一次排查：剪贴板
问题复现了，证据却已经没了。现在改为追加、超 4 MB 才滚动、启动横幅带 pid。

**`exit` 就是 `exit`：主动退出的 shell 不再自动重连。** 敲完 `exit` 一秒后又连上，这个
命令就没有意义了。机制本来就在——`ReconnectState::manual` 的注释写着"用户主动断开*或
shell 退出*；不要重连"——只是从没为一次普通的 `exit` 触发过。

它靠 `ChannelMsg::ExitStatus` 拼出的状态字符串前缀来判断。而服务端通常先发 `Eof`，读循环
在 `Eof / ExitStatus / Close` 三者的第一个就 break，于是退出码往往根本没被读到，那条字符
串也就从没发出去。**用格式化字符串承载控制流，正是这个失败能一直隐形的原因。**

现在由 shell 层显式发一个 `ShellExited` 事件——只有读通道的那一侧分得清这两种情况。划界
是：`Eof` / `Close` / `ExitStatus` 都表示远端主动结束，不重连；**流直接断掉、没有任何告
别**才是掉线，照常重连。`Ok(None)` 因此从原本和 `Eof | Close` 共用的分支里拆了出来——它们
看着像一回事，含义正好相反。

**`exit` 之后整个会话卡死。** 上一条的前提。这是一个精确的循环等待：

```
run_live_password_shell:  shell 结束 → 等所有 SharedSession 释放 → 发 Closed
会话层:                    收到 Closed → 释放自己的 SharedSession
```

会话层从 `SessionReady` 起持有那份 claim（好让 SFTP／隧道搭车），而释放它的代码就在**永远
不会执行的** `Closed` 分支里。回车不能重连并不是第二个 bug——重连的前提是会话被标记为已断
开，而没有任何东西标记过它。**一个死锁造成了两个现象。**

修法是先宣告、再等待：`Closed` 在 shell 结束时立刻发出。两个性质都保住了——UI 立即得知，
连接仍等到最后一个使用者释放才真正断开，只是这个等待不再卡住它自己依赖的那个通知。

回归测试**像会话层那样持有 `SharedSession`** 再断言 `Closed` 仍会到达。持有这一点是全部
要害：不持有的话 claim 会自行释放、等待自然结束、bug 根本复现不了——这也正是它此前没被任何
测试抓到的原因。修复前后各跑一次确认过（无修复超时失败，有修复通过）。

**标签栏：去掉重复状态，`+` 跟在最后一个标签后面。** 连接状态原本紧挨着写有同一个会话名
的标签，而状态栏和标题栏都已经在显示它——三份同样的字符串，被去掉的是占了这一行最有用位置
的那一份。`+` 直接复用侧边栏「新建会话」按钮本身（同字形、同提示、同消息），两处不会漂移
成不同含义。

**顺带修了一串对不齐。** 根因是 **iced 不会替你居中**：`Button::layout` 走
`layout::padded`，其定位步骤是 identity；`Container` 默认 `Vertical::Top`；`Row` 默认
`Alignment::Start`。**给一个东西设高度，并不会摆放它里面的内容。** 于是 34 高的框里装 15
高的文字，15 全部贴在顶部、空白全在下面。同一行里还挤着三种高度（主机 34、会话标签由内容
撑开、`+` 26），侧边栏表头又是固定 30 对着标签栏的 34。现在 `TAB_HEIGHT` 统一了行内高度，
三处一律显式居中。

**设置里的快捷键说明页。** 界面从不提及的那些键，此前只能靠读源码找到——`Alt+P`
（SFTP 命令行）尤其典型，它存在，而唯一提到它的地方是一句你得先触发才能看到的错误
提示。

每一条都是先在 `update_loop` 的按键分发里找到依据才写上去的。这是这种页面的全部价值
所在，也是它唯一会腐烂的地方：列出一个按了没反应的键，比不列更糟——读者会断定功能坏
了，而不是没有。

去核对而不是凭记忆，当场抓到两件事。**`Alt+R` 根本不存在**：分发里的一句注释宣称它跳
到工具栏的主机框，而从来没有任何代码匹配过它；它差一点就被抄成一句应用兑现不了的承诺。
**回滚跳转是 `Ctrl+Shift+Home/End` 而不是 `Ctrl+Home`**：`terminal_scroll_shortcut`
在没按 Shift 时直接返回，所以下面那两条 Ctrl 分支实际要求两个修饰键同时按。

已知限制（写在代码里）：这份清单是手写常量，和分发之间没有任何机制保证同步。今天是准
的；以后改了快捷键却没动它，它就开始撒谎。让两者共用一份数据才是根治，那是更大的改动。

**多选之后可以一次拖到另一个分组。** 报上来的现象是多选完拖不动。原因是按下时就把选中
塌成了被按的那一行——指针还没移动，用户刚建立的选中就没了，之后的拖拽只带着一个会话。

现在按在选中集合*内*的行不再改动选中；按在集合*外*的行仍然立刻替换（没什么可保留的，
而拖动全程留着错误的高亮更糟）；按下后没拖成的仍然是普通点击，塌陷推迟到松手时发生——
否则点击就再也无法缩小选中，那会是个比原 bug 更糟的 bug。

**所有落点都照做，不只是报上来的那个分组。** 只修分组会得到一个比 bug 更奇怪的结果：
选中活了下来，却只移动一行。拖到某行旁边和网格里用**锚点行走**——第一行落在落点处，后
续每行落在前一行之后，于是一批到达时是连成一片且保序的；顶层那条必须带上偏移量，否则
每个都放 index 0 会让它们倒序落地，那看起来像 bug 而不像设计。

**按配置的「退出 shell 后保留 SSH 连接」。** MFA 主机上，`exit` 之后再开 SFTP 或隧道
就连不上了。查下来的因果链比 roadmap 原本写的更具体：SFTP／隧道**有现成会话时搭车**，
没车时自建；自建路径其实*会*走 keyboard-interactive，只是 `InteractiveKbd` 传的是
`None`，没有向用户提问的通道，只能拿存着的密码尽力填——所以只用密码的
keyboard-interactive 主机一直是通的，坏的只有真正要第二因素的那类。

而连接之所以会没，是 `LiveShellEvent::Closed` 里主动释放了 `shared_session`。那一行是
**故意**的，注释写着理由：不释放的话 `exit` 会留下一条已建立的连接。所以这里没有免费
的修法——只能把那个取舍交给用户，于是做成按配置的开关，默认关。

两个决定：

**标志缓存在 `SessionRecord` 上，而不是关闭时回查 profile。** 那时 `record` 已被可变
借用，回头去 `self.profiles` 查不过借用检查——`auto_reconnect` 在循环前被捕获成局部
变量，正是同一个原因。顺带得到一个更好的性质：会话中途改配置，不会改变一条已经按旧设置
建立起来的连接的下场。

**没有给自建路径接提问通道。** 技术上可行，但那意味着每条新隧道都要一次新验证码，而
TOTP 同一时间窗内通常复用不了——多半只是把"连不上"换成"输了也连不上"。

复选框的文案是围绕**代价**写的而不是围绕功能："保留连接"听起来是免费的，实际被同意的
是一条通往生产机器的、已认证的 SSH 连接在有人敲完 `exit` 之后继续挂着。它也计入「高级」
折叠头上的"已设置 N 项"，否则折叠起来就看不出这个配置不寻常了。

**SFTP 覆盖确认。** 传输一个目标位置已有的同名文件，之前是静默覆盖——而 SFTP 没有
回收站，被替换掉的文件就是没了。旁证是这件事本来就不自洽：删除一直有确认栏，覆盖
没有。

检查放在一个函数里，四个入口全部收敛过去（右键菜单、面板之间拖拽、文件选择器、手
输路径），因为这种防护真正的失效方式不是判断写错，而是日后多出第五个入口悄悄绕过
去。整批一起拦：拖十个文件其中一个冲突，就问一次，答完按原样执行；没有做逐个"跳过"，
两个按钮和删除栏的两个含义完全一致，比多一个没人要的选项更值。

只对文件生效。目标位置同名的是*目录*时不算覆盖——传输本来就会失败而不会毁掉什么——
提示"确认覆盖"反而是承诺了一件不会发生的事。

**它收窄了窗口，没有关上。** 判断依据是面板正在显示的那份列表，而不是每次传输前去
服务器 stat 一次：这样匹配用户眼睛看到的东西，代价是上次刷新之后才出现的文件会漏掉。

顺带记一笔：roadmap 里"身份文件只能敲、没有浏览按钮"那条是陈旧的，编辑器里的
「浏览…」按钮早就在了。我在推荐它之前声称"读了代码确认过"，实际上 grep 的输出在按钮
那一行之前就被截断了——一次本可以避免的错误结论。
