...
- **英文翻译只做了一半。** 菜单、设置页、各种对话框和运行时提示都覆盖了；缺失的
  ...
  一个好消息是这张表的**重复条目是编译期能抓到的**：同一句中文写两遍会触发
  `unreachable_patterns`，而 CI 的 `-D warnings` 会把它变成错误（加快捷键页时就撞到过
  一次）。所以真正会静默出错的只有"该说得不一样却说不出来"那一半，不包括"不小心写重了"。
...
- **剪贴板不支持拖放。** 图片（`CF_DIB`）双向已经通了，拖放没有：它是另一套 shell
  交互，和复制粘贴不共用任何东西。
- **全局热键只在全屏时透传。** 窗口模式下，别的程序用 `RegisterHotKey` 注册的热键
  （截图工具的 `Ctrl+Shift+X` 之类）依然被本地抢走。这是故意的，和 mstsc 一致：低级
  键盘钩子对整台机器生效，只有远端明确占满整个屏幕时，把全机器的按键截下来才算划
  得来。
- **RDP 画质是全应用一份设置。** 和它旁边的剪贴板开关一样，没有按会话覆盖：
...
- **MFA 主机上，自建连接仍然答不了第二因素。** SFTP、隧道和 SFTP 协议在有现成 SSH
  会话时会搭车复用，不需要回答任何 challenge。没车可搭时它们会自己拨一条——那条路径
  *会*尝试 keyboard-interactive，只是没有向用户提问的通道（`InteractiveKbd` 传的是
  `None`），所以只能用存着的密码尽力填。因此只用密码的 keyboard-interactive 主机本来
  就是通的，坏的只有真正需要额外因素的那一类。

  按配置的「退出 shell 后保留 SSH 连接」（默认关）覆盖了实际会撞上的场景：连接不再随
  shell 退出而关闭，之后开的 SFTP／隧道仍有车可搭。没做的是给自建路径接上提问通道——
  那意味着每条新隧道都要一次新验证码，而 TOTP 在同一时间窗内通常复用不了，所以那条路
  多半只是把"连不上"换成"输了也连不上"。
...
- **隧道扛不住自己那条连接掉线。** 每条隧道跑自己的 SSH 连接，连接一死隧道就停，
  而不是重拨。
- **SFTP 传输没有"另存为下载"，也没有续传。** 覆盖确认有了：它按面板正在
  显示的那份列表判断，所以只收窄了窗口而没有关上——上次刷新之后才出现的文件仍会漏掉
  （已合入 main，尚未发版）。
- **窗格不能拆出**到独立窗口再合回来。
