...
pub fn decode_rlex(data: &[u8]) -> DecodeResult<RlexData> {
    ...
    // ADIT PATCH: a one-entry palette still uses one stop-index bit.
    //
    // FreeRDP computes `numBits = CLEAR_LOG2_FLOOR[paletteCount - 1] + 1`, and
    // its table has `CLEAR_LOG2_FLOOR[0] == 0`, so a single-entry palette
    // yields ONE bit, not zero — the normal (packed, runLength) pair is still
    // read per segment, with the stop index forced to 0 by the range check.
    // This crate special-cased it to zero bits and read one byte per segment,
    // so every byte of a single-colour RLEX region was misread, the segment
    // count roughly doubled, and the cumulative suite count overshot the
    // region — surfacing as "suite exceeds region pixel count" (16 of those in
    // one session). For palette_count >= 2 the two agree, since
...
#[expect(dead_code, reason = "orphaned by the ADIT PATCH above; kept for comparison with upstream")]
