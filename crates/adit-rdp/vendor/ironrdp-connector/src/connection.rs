    ...
    fn step(&mut self, input: &[u8], output: &mut WriteBuf) -> ConnectorResult<Written> {
        let (written, next_state) = match mem::take(&mut self.state) {
            ...
            ClientConnectorState::ConnectionInitiationSendRequest => {
                ...
                // ADIT PATCH: GNOME Remote Desktop's system-mode ("Remote Login")
                // handover redirects us with a routing token and expects RDSTLS
                // (one-time credentials) rather than CredSSP/NLA. When a routing
                // token is present we request RDSTLS exclusively — this forces the
                // handover daemon down the RDSTLS path (or fails negotiation cleanly)
                // instead of falling back to a protocol we then wouldn't authenticate.
                // The caller runs the RDSTLS exchange on the TLS stream before MCS.
                // See IRONRDP-PATCHES.md.
                let rdstls_redirect = matches!(
                ...
                if !rdstls_redirect && self.config.enable_tls {
                    security_protocol.insert(nego::SecurityProtocol::SSL);
                }

                if !rdstls_redirect && self.config.enable_credssp {
    ...
    Ok(ClientGccBlocks {
        core: ClientCoreData {
            ...
            optional_data: ClientCoreOptionalData {
                ...
                early_capability_flags: {
                    // ADIT PATCH: dropped SUPPORT_NET_CHAR_AUTODETECT (paired with
                    // `message_channel: None` below), and added
                    // SUPPORT_DYN_VC_GFX_PROTOCOL — the client must advertise
                    // Graphics Pipeline support here, during capabilities exchange,
                    // or servers that mandate EGFX (e.g. GNOME Remote Desktop) reject
                    // the connection. Adit opens the RDPGFX DVC to back this up.
        ...
        cluster: None,
        monitor: None,
        // ADIT PATCH: was `Some(gcc::ClientMessageChannelData)`. Requesting the
        // message channel makes the connector enter its ConnectTimeAutoDetection
        // phase, where it only advances to licensing on an I/O-channel PDU. Some
        // Windows servers send message-channel PDUs there that IronRDP's
        // AutoDetectReqPdu decoder rejects; it then ignores them and waits forever
        // for a licensing PDU the server won't send until we respond — a deadlock
        // (reproduced against a real Win host, 2026-07-12; present in 0.10.0/master).
        // Not requesting the message channel skips that phase entirely (the code
        // below: "Without a message channel ... we go straight to licensing"). We
        // lose optional network auto-detect / UDP multitransport / heartbeat, which
        // Adit doesn't use anyway.
