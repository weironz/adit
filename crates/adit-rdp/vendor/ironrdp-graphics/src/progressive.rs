...
fn dequantize_component_ccq(coefficients: &mut [i16], quant: &ComponentCodecQuant, use_reduce_extrapolate: bool) {
    ...
    for (band_idx, band) in bands.iter().enumerate() {
        let q = quant.for_band(band_idx);
        // ADIT PATCH: the dequantization shift is `quant - 6`, not `quant - 1`.
        //
        // MS-RDPRFX 3.1.8.1.4 / FreeRDP `rfx_quantization_decode_block`, which
        // is called with `quantVals[i] - 6`. Upstream subtracted 1, so every
        // coefficient came out 32x too large: the inverse DWT then pushed all
        // three planes past the YCbCr→RGB clamps and a photograph rendered as
        // flat black, red, yellow and white — the whole desktop, not a corner
        // of it. Verified against a captured Windows stream: with `- 1` the
        // lock screen decodes to coloured garbage, with `- 6` it decodes to the
        // photograph, pixel-clean. See tests/progressive_dump.rs.
        let factor = i16::from(q).saturating_sub(6);
        if factor > 0 {
            let start = band.offset;
            let end = start + band.count();
            for coeff in &mut coefficients[start..end] {
...
fn decode_tile_block(
    ...
    match tile_block {
        ...
        ProgressiveTile::First(tile) => {
            ...
            //
            // MS-RDPRFX 2.2.4.3.6: a `quality` of 0xFF means the tile carries no
            // progressive quantization at all — it is encoded losslessly — and
            // must NOT be looked up in `quantProgVals`. Windows sends exactly
            // that, with `numProgQuant` 0, so every tile of every frame failed
            // with "quant index 255 exceeds table length 0" and the desktop
            // rendered as coloured garbage. GNOME Remote Desktop and xrdp send
            // real indices, which is why upstream never hit this.
            //
            // The TILE_SIMPLE path already uses ComponentCodecQuant::LOSSLESS
            // for the same reason; this is the missing half of it.
            const LOSSLESS_QUALITY: u8 = 0xFF;
            // All-zero progressive quant: nothing is dropped, which is what
            // "losslessly encoded" means for the bit-plane stages downstream.
            const LOSSLESS_PROG: ProgressiveCodecQuant = ProgressiveCodecQuant {
            ...
            let pq = if tile.quality == LOSSLESS_QUALITY {
                &LOSSLESS_PROG
            } else {
...
mod tests {
    ...
    fn dequantize_component_ccq_shifts_correctly() {
        ...
        // ADIT PATCH: was asserting the `quant - 1` shift this crate used to
        // apply. Both quant values here are below 6, so the correct shift is
        // zero and neither coefficient moves.
        // HL1: 4 - 6 saturates to 0 -> unchanged.
        assert_eq!(coefficients[0], 10);
        // LL3: 3 - 6 saturates to 0 -> unchanged.
